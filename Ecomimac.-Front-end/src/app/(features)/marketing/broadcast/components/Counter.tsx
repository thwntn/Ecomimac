...
import { concatPathName, formatNumber, RouteMap } from "@/utils/common"
import { useGet } from "@/utils/hooks"
import { CounterBroadcastResponse } from "@/utils/interface"
import { Fragment } from "react"

const Counter = () => {
  const fetchCounter = useGet<CounterBroadcastResponse>(
    concatPathName(RouteMap.BROADCASTS, RouteMap.COUNTER)
  )

  const isLoad = convert(fetchCounter.isWait === true)
  return (
    <Row gap={16}>
          ...
          render={(counter) => (
            <Fragment>
              ...
              <Row itemsCenter gap={4}>
                Có
                <span className="text-green-500">{counter.runningCount}</span>
